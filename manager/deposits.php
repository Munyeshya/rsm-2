...
if (isset($_POST['submit'])) {
    ...
    $date=date("Y/m/d");
    $typy=$_POST['type'];
    $query=$conn->query("INSERT INTO deposits(depositor,amount,date) values('$typy','$cash','$date')") or die(mysqli_error($conn)) ;
    if ($query) {
        $alertmess="Record Successfull !!".$conn->error;
        $alertclass="alert alert-primary";
    }else{
        $alertmess="Record Unsuccessful !!";
        $alertclass="alert alert-danger".$conn->error;
    }
}
 ?>
<!DOCTYPE html>
 			...
 			<div class="well shadow-sm bg-white p-2">
 				<div class="row">
 					 ...
 					 <div class="col-md-5"><br>
 					 	<h4 class="text-center font-italic font-weight-normal">Deposits:</h4>
 					 </div>
 				</div><br>
...
<div id="pdf" >
   ...
   <label><b>DEPOSITS REPORT</b></label><br>
<label><b>BY: <?php echo $wel?></b></label> <br>  
&nbsp;&nbsp;&nbsp;&nbsp;
   ...
   if(isset($_POST['print'])){
       $edate=$_POST['edate'];
       $sdate=$_POST['sdate'];
       $query=$conn->query("SELECT * from deposits where date BETWEEN '$sdate' and '$edate'  ");
       $bring = mysqli_query($conn,"SELECT *  from deposits where date BETWEEN '$sdate' and '$edate' ");
       $bring2 = mysqli_query($conn,"SELECT sum(amount) as sum  from deposits where date BETWEEN '$sdate' and '$edate' ");
