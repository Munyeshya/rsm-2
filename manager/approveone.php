...
if ( empty($paye) ) {
    ...
    if ( $pay <= $o ) {
        $left=$o-$pay;
        $inserte=$conn->query("INSERT into money(names,orderid,pay1,method1,date,reste) VALUES('$fullname','$id','$o','$method','$date','$left') ") or die(mysqli_error($conn));
...
elseif( !empty($paye)  ){
    if ($pay <= $reste ) {
        ...
        $finale=$conn->query("UPDATE money set reste='$left' where orderid='$id' ")or die(mysqli_error($conn));
        // code...
        
        $fin=mysqli_query($conn,"SELECT SUM(pay1) as pay1  from moneyhist where orderid='$id'");
        $fins=mysqli_fetch_array($fin);
        $finally=$fins['pay1'];
        
        $upd4=mysqli_query($conn," UPDATE money set tot='$finally' where orderid='$id' ");
    ...
    <div class="row">
        ...
        Contact:<?php echo $con ?>
            ...
            <th>
                ...
                <a href="cloud.php?order=<?php echo $id ;?>" class="text-info" style="text-decoration:none;"><i class="icofont-paper-plane icofont-2x"></i></a>
            </th> 
            <th>
...
?>
<em>
Payment :&nbsp<b><?php echo number_format($maersk['pay1']) ?> FRW</b> <br>
...
?>
Left:&nbsp<b><?php echo number_format(@$lefties['reste']) ?> FRW</b>
